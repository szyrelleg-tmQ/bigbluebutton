...
import { Message, TranslationMessage, AccumulatingTranslationMessage } from '/imports/ui/Types/message';
...
import { latestTranscriptionVar, latestTranslationVar, getFilteredTranslationMessages, translationMessagesVar, currentAccumulatingTranslationVar } from '/imports/ui/services/audio-manager';
...
const ChatMessageList: React.FC<ChatListProps> = ({
  ...
  const filteredTranslationMessages = useReactiveVar(translationMessagesVar) as TranslationMessage[];
  const currentAccumulatingTranslation = useReactiveVar(currentAccumulatingTranslationVar) as AccumulatingTranslationMessage | null;
        ...
        [
          ...
          >
            ...
            >
              ...
              {(filteredTranslationMessages.length > 0 || currentAccumulatingTranslation) && (
                <div style={{ margin: '24px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {/* Render accumulating message if exists */}
                  {currentAccumulatingTranslation && (
                    <div
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      <ChatAvatar
                        avatar={''}
                        color={getConsistentAvatarColor(undefined, currentAccumulatingTranslation.participant_name || 'User')}
                        moderator={false}
                        style={{
                          marginRight: 0,
                          width: 48,
                      ...
                      >
                        {(currentAccumulatingTranslation.participant_name || 'U').slice(0, 1).toUpperCase()}
                      </ChatAvatar>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 500, fontSize: 15, color: '#222', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ color: '#888', fontWeight: 400, fontSize: 13 }}>{intl.formatMessage({ id: 'app.chat.originalLabel', defaultMessage: 'Original:' })}</span>
                          <span style={{ wordBreak: 'break-word', fontWeight: 500 }}>{currentAccumulatingTranslation.original}</span>
                          <span style={{ color: '#ff9800', fontSize: 12, fontWeight: 400 }}>(Accumulating...)</span>
                        </div>
                        <div style={{
                          background: '#fff3e0',
                          borderRadius: 8,
                          ...
                          color: '#e65100',
                          marginBottom: 0,
                          boxShadow: '0 1px 2px rgba(255,152,0,0.04)',
                          wordBreak: 'break-word',
                          display: 'flex',
                          alignItems: 'center',
                          gap: 8,
                        }}>
                          <span style={{ color: '#f57c00', fontWeight: 400, fontSize: 13 }}>{intl.formatMessage({ id: 'app.chat.translatedLabel', defaultMessage: 'Translated:' })}</span>
                          <span>{String(currentAccumulatingTranslation.translations?.[userLang] || currentAccumulatingTranslation.original)}</span>
                        </div>
                      </div>
                    </div>
                  )}
